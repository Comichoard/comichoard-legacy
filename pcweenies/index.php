<?php
    $url = 'http://'.$_SERVER['HTTP_HOST'].'/pcweenies/pcweenies.php?';
    $source = 'pcweenies';
    $strip = $_GET['strip'];
    if(isset($_GET['strip']))
        ...
        <script>
            ...
            $(window).scroll(function () {
                if ($(window).scrollTop() + $(window).height() > $(document).height() - 4000 && flag == 0) {
                    flag = 1;
                    $.post("pcweenies.php?comic=" + next, function (e) {
            ...
            $(document).on('click','.s',function (event) {
                ...
                window.open('https://www.facebook.com/sharer/sharer.php?u=' + encodeURIComponent('http://' + $('#website').val() + '/pcweenies/?strip=' + $(this).attr('data-share')));
            });
            ...
            $(document).on('keydown','#comicnumselect',function(event) {
                if(event.which == 13)   {
                    ...
                    $(".panel-body").append('<div class="jumbotron cdesc">'+retain+'</div>');
                    $(".panel-body").append('<div id="loadmsg" class="jumbotron">Stay Calm and Wait for More</div>');
                    $.post("pcweenies.php?comic=" + btoa($(this).val()), function (e) {
                        next = e.split("!znavfu")[0];
                        e = e.split("!znavfu")[1];
                        e = e.split("<!--")[0];
                        if (e.split("script").length == 1)
