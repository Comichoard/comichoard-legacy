<?php
    ...
    $url = 'http://'.$server.'/pennyarcade/pennyarcade.php?';
    $source = 'pennyarcade';
...
<html lang="en">
    ...
    <body>
        <div id="viewer" class="panel panel-default">
            ...
            <div id="footer" class="footer">Help your friends see how awesome Penny Arcade is too. <a href="https://www.facebook.com/sharer/sharer.php?u=http%3A%2F%2Fcomichoard.com%2Fpennyarcade" class="btn btn-default begsuccess btn-sm" target="_blank">Share Penny Arcade</i></a></div>
        </div>
        ...
        <script>
            ...
            $(window).scroll(function () {
                if ($(window).scrollTop() + $(window).height() > $(document).height() - 4000 && flag == 0) {
                    flag = 1;
                    $.post("pennyarcade.php?comic=" + next, function (e) {
            ...
            $(document).on('click','.s',function (event) {
                ...
                window.open('https://www.facebook.com/sharer/sharer.php?u=' + encodeURIComponent('http://' + $('#website').val() + '/pennyarcade/?strip=' + $(this).attr('data-share')));
            });
            ...
            $(document).on('keydown','#comicnumselect',function(event) {
                if(event.which == 13)   {
                    ...
                    $(".panel-body").append('<div class="jumbotron cdesc">'+retain+'</div>');
                    $(".panel-body").append('<div id="loadmsg" class="jumbotron">Stay Calm and Wait for More</div>');
                    $.post("pennyarcade.php?comic=" + btoa($(this).val()), function (e) {
                        next = e.split("!znavfu")[0];
                        e = e.split("!znavfu")[1];
                        e = e.split("<!--")[0];
                        if (e.split("script").length == 1)
