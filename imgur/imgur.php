<?php
    ...
    function getcomic($i)   {
        ...
        array_push($sendback, '<div class="card">'.$imgstring.'<div class="details"><span>'.ucwords($name[0]).'</span><!--span class="fb-like" data-layout="button_count" data-action="like" data-show-faces="false" data-share="true" data-href="http://comichoard.com/'.$comic.'/?strip='.$url.'">Share</span--></div></div>');
    ...
    if(isset($_GET['comic'])) {
        ...
        if(isset($_GET['strip']))   {
            getcomic($_GET['strip']);
            array_push($sendback,'<div class="jumbotron">More comics from imgur...</div>');
            $count--;
        }
        $url=getcomic(0);
        echo base64_encode($url).'!znavfu';
        echo '<div class="jumbotron cdesc"><h1>Best of Imgur <a href="http://imgur.com/" type="button" class="btn btn-default" target="_blank">Go to site</a></h1>
